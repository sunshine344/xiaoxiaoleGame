...
export function useGame(config: GameConfig): Game {
	...
	function handleSelect(node: CardNode) {
		...
		if (node.parents.length != 0) {
			let isClicked = node.parents.findIndex((it) => it.state != 2);
			if (isClicked != -1) {
		...
		if (index > -1) delNode && nodes.value.splice(index, 1);
		// 判断是否有可以消除的节点
	...
	function initData(config?: GameConfig | null) {
		...
		const width = cardNum == 25 ? containerWidth / 2 + 80 : containerWidth / 2;
		const height = cardNum == 25 ? containerWidth / 2 + 60 : containerHeight / 2 - 60;
